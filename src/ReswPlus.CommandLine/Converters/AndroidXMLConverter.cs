...
public class AndroidXMLConverter
{
    ...
    {
        var groupedItems = supportPluralization
            ? (from item in reswInfo.Items let simplifiedKey = ExtractReswKey(item.Key) group item by simplifiedKey into groupItem select groupItem).ToDictionary(k => k.Key, k => k.Select(j => j).ToList())
            : reswInfo.Items.ToDictionary(p => p.Key, p => new List<ReswItem>() { p });
        var androidXML = new XmlDocument();
        var resourcesNode = androidXML.CreateElement("resources");
        _ = androidXML.AppendChild(androidXML.CreateComment("smartling.instruction_comments_enabled = on"));
        _ = androidXML.AppendChild(androidXML.CreateComment(@"smartling.placeholder_format_custom = \{\d\}"));
        _ = androidXML.AppendChild(resourcesNode);
                ...
                case 1:
                    {
                        ...
                        {
                            _ = resourcesNode.AppendChild(androidXML.CreateComment(valueNode.Comment));
                        }

                        var stringNode = androidXML.CreateElement("string");
                        stringNode.SetAttribute("name", resourceItem.Key);
                        _ = stringNode.AppendChild(androidXML.CreateTextNode(valueNode.Value));

                        _ = resourcesNode.AppendChild(stringNode);
                    }
                    break;
                default:
                    {
                        ...
                        foreach (var reswItem in resourceItem.Value)
                        {
                            ...
                            {
                                _ = pluralsNode.AppendChild(androidXML.CreateComment(reswItem.Comment));
                            }
                            ...
                            _ = itemNode.AppendChild(androidXML.CreateTextNode(reswItem.Value));
                            _ = pluralsNode.AppendChild(itemNode);
                        }
                        _ = resourcesNode.AppendChild(pluralsNode);
                    }
                    break;
            }
        }
